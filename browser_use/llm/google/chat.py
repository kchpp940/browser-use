		...
		async def _make_api_call():
			...
			try:
				if output_format is None:
					...
					# Collect function calls from parts for normalization
					def _build_normalized(resp: types.GenerateContentResponse, fmt: type[T]) -> NormalizedLLMResponse:
						raw_text = resp.text or ''
						parsed_dict: dict[str, Any] | None = None
						tool_calls: list[NormalizedToolCall] = []

						# If native parsed is available, use it
						if resp.parsed is not None:
							try:
								if isinstance(resp.parsed, dict):
									parsed_dict = resp.parsed
								elif not isinstance(resp.parsed, bytes) and hasattr(resp.parsed, 'model_dump'):
									parsed_dict = resp.parsed.model_dump()  # type: ignore
								elif isinstance(resp.parsed, BaseModel):
									parsed_dict = resp.parsed.model_dump()
							except Exception:
								parsed_dict = None

						# Also extract function_call / function_calls from parts
						if hasattr(resp, 'candidates') and resp.candidates:
							...
							if hasattr(candidate, 'content') and candidate.content is not None:
								...
								for part in parts:
									...
									if fc is not None:
										fname = str(getattr(fc, 'name', fmt.__name__))
						...
						return NormalizedLLMResponse(
							...
							stop_reason=self._get_stop_reason(resp),
						)
					...
					usage = self._get_usage(response)
					normalized = _build_normalized(response, output_format)
					try:
